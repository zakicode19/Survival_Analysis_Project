...
```
`r surv_pvalue(fit.hbp,data)$pval`, there is an effect of hbp in time to heart failure.  
...
### blood pressure  
```{r}
hbp_fit <- survfit(Surv(time, DEATH_EVENT) ~ high_blood_pressure , data = data)
if (surv_pvalue(hbp_fit,data)$pval <= 0.5) {
   ggsurvplot(hbp_fit,
            #fun = "event",
           #conf.int=TRUE, 
           pval=TRUE,
           ...
           title="Difference in survival time in high blood pressure", 
           #risk.table.height=.28, 
           data=data)
} else {
  cat("p-value = ",surv_pvalue(hbp_fit,data)$pval,"There no difference in survival time in hbp")
}
```
...
```{r,eval=FALSE}
ggsurvplot(anaemia_fit, conf.int=TRUE, pval=TRUE, risk.table=TRUE, 
...
if (surv_pvalue(age.fit,data)$pval <= 0.5) {
  ...
  cat(" p-value = ",round(surv_pvalue(age.fit,data)$pval,2), "There is no difference in survival time in in age over 60")
}
```

### additive effect of Ejection Fraction  
```{r}
ejection_fit <- survfit(Surv(time, DEATH_EVENT) ~ EF_levels , data = data)
if(surv_pvalue(ejection_fit,data)$pval  <= 0.05) {

ggsurvplot(ejection_fit,
           #fun = "event",
           pval = TRUE , data = data, 
           legend.title="EF levels",
           title="Difference in Ejection Fraction survival", 
           )
} else {
 cat("p-value = ", surv_pvalue(ejection_fit,data)$pval," There is no 
