...
```{r, echo=FALSE, message=FALSE, results=FALSE, warning=FALSE, fig.height=4,fig.width=6}
...
ggsurvplot(fit.KM,data=data, conf.int=TRUE, pval=TRUE, #risk.table=TRUE, risk.table.height=.10,
           ...
           title="Kaplan-Meier Curve for Heart Failure Survival", xlab = "Day", ggtheme = theme
           )
```
...
ggsurvplot(fit.KM1, conf.int=TRUE, pval=TRUE,
           ...
           title="Kaplan-Meier Curve per EF levels for Heart Failure Survival", xlab = "Day", pval.size=4,
           legend=c(0.15,0.25),pval.coord=c(0,0), ggtheme = theme,
           data=data)
```
...
ggsurvplot(fit.KM2, conf.int=TRUE, pval=TRUE,
           ...
           pval.size=4,legend=c(0.15,0.25),pval.coord=c(0,0), ggtheme = theme,
           data=data)
```
...
```
A long-rank test was conducted to determine whether there are differences in survival between groups on each of the categorical covariates. The results are summarized in the following table:

|covariate |  Sex     | Smoking  | Diabetes | Aenemia |EF levels |bad platelet|Age over 60|Blood Pressure|  
|----------|----------|----------|----------|---------|----------|------------|-----------|--------------|
| p-value  |  0.95    | 0.96     |  0.84    | 0.099   | 1.81e-07 |     0.26   |    0.02   |    0.05      |
...
library(knitr)
knitr::kable( as.data.frame(updated_df))
...
### Cox semi-parametric model : additive effect of age and anaemia.  
...
\newpage
##  Parametric model 
We fit a weibull parametric model and compare it to non parametric KM model.  
...
plot(fitparam,
     frame = F,
     col = "red",
     type = "survival", main= "weibull model")
abline(h = 0.5, lty="dashed",col ="blue")
plot(fit.KM,col = "green", frame = F,main = "KM model")
abline(h = 0.5,lty="dashed",col ="blue")
```
This plot suggest that we could model this data under a linear model with a negative slope. As it decreases very fast. However, this not a stepwise or strictly smooth curve. So we tried an exponential model .The weibull model curve is plotted against KM model. Though, KM model could suffice to model this dataset, we would suggest.  
...
We aim at analyzing time to heart failure.We have found that, there is an effect of high blood pressure , EF , serum_creatinine, serum_sodium and ageing, but surprisingly no effect of Smocking, nor diabetes or anaemia.  following one could consider that, this study held in a cohort of patients of NYHA class III and IV which are advanced stages of heart failure. Similar results concerning diabetes and smoking have been reported by [F Otero-Raviña et al. 2009](https://scholar.google.com/scholar?q=Morbidity+and+mortality+among+heart+failure+patients+in+Galicia%2C+N.W.+Spain%3A+the+GALICAP+study+Otero-Raviña+2009) as well. 
