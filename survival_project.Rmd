...
```{r, echo=FALSE,  warning=FALSE}
library(broom)
coefs <- tidy(fit_cph, exponentiate = TRUE)  # Set exponentiate to TRUE to obtain hazard ratios instead of coefficients
# Print the coefficients and p-values table
knitr::kable( as.data.frame(coefs))
```

```{r, echo=FALSE, warning=FALSE}
test.ph <- cox.zph(fit_cph)
knitr::kable( as.data.frame(test.ph$table))
```
```{r, echo=FALSE, warning=FALSE}
ggcoxzph(test.ph, ggtheme = theme)
```
